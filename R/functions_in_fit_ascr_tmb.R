...
mask.fun = function(mask, dims, data.traps, data.full, bucket_info, local,
  ...
  for (i in 1:n.sessions){
    ...
    dists[[i]] <- ascr::distances(as.matrix(traps), mask[[i]])
    ...
    thetas[[i]] = ascr::bearings(as.matrix(traps), mask[[i]])
    ...
    if (is.null(attr(mask[[i]], "area"))){
      mask.dists <- ascr::distances(mask[[i]], mask[[i]])
  ...
  if ("mrds" %in% bucket_info){
    ...
    for (i in 1:n.sessions){
      ...
      if(n.IDs[i] > 0){
        ...
        all.which.local[[i]] <- ascr::find.nearest.mask(as.matrix(mrds), mask[[i]])
        all.n.local[[i]] <- ascr::laply(all.which.local[[i]], length)
    ...
    for (i in 1:n.sessions){
      if(n.IDs[i] > 0){
        ...
        all.which.local[[i]] <- ascr::find_local(bincapt, dists[[i]], buffer[i])
        all.n.local[[i]] <- ascr::laply(all.which.local[[i]], length)
        all.which.local[[i]] <- c(all.which.local[[i]], recursive = TRUE)
      }
    }
  ...
  for(i in 1:n.sessions){
    if(n.IDs[i] > 0){
      ...
      if('toa' %in% bucket_info){
        ...
        toa.ssq = ascr::make_toa_ssq(toa, dists[[i]], sound.speed)
  ...
  if(fit.ss){
    ...
    if(het.source){
      if(het.source.method == "GH"){
        GHd <- ascr::gaussHermiteData(n.het.source.quadpoints)
