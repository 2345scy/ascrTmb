p.dot.defaultD = function(points = NULL, traps = NULL, detfn = NULL, ss_dir = NULL,
  ...
  dists <- ascr::distances(traps, points)
  ...
  if(ss_dir){
    ...
    probs <- ascr::calc.detfn(dists, detfn, pars, ss.link)
    out <- ascr::aaply(probs, 2, function(x) 1 - prod(1 - x))
  }
